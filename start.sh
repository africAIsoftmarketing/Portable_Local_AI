...
# Prompts for model selection when multiple .gguf files exist,
# then starts the llama.cpp web server.
#
# IMPORTANT: llama-server depends on shared libraries (libllama.so, libggml.so, etc.)
# that are installed alongside it in the bin directory by install.sh.
# We set LD_LIBRARY_PATH (Linux) / DYLD_LIBRARY_PATH (macOS) to the bin dir
# so the binary always finds them regardless of system configuration.
...
# ── Pick binary path based on OS / Architecture ───────────────────────────────
OS="$(uname -s)"
ARCH="$(uname -m)"
...
# ── Set library path so .so files next to the binary are found ────────────────
# llama-server links against libllama.so, libggml.so, libggml-cpu.so etc.
# install.sh places all of them in BIN_DIR alongside the binary.
case "$OS" in
    Linux*)
...
echo -e " ${GREEN}[+] Local   :${NC} http://127.0.0.1:8080"
...
# ── Start server ──────────────────────────────────────────────────────────────
# Note: --path ui is optional — llama-server has the web UI embedded.
# We pass it anyway so a custom ui/ folder is used if it exists.
exec "$BIN" \
    -m "$MODEL" \
    -c 4096 \
    -t "$THREADS" \
